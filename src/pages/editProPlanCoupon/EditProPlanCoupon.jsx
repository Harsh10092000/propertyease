...
const EditProPlanCoupon = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setCouponData({
            ...
            pro_coupon_amt: res.data[0].coupon_amt,
            pro_coupon_valid_form: res.data[0].coupon_valid_form,
            pro_coupon_valid_till: res.data[0].coupon_valid_till,
            coupon_id: res.data[0].coupon_id,
        });
        setTransactionDate(
            dayjs(
              moment(res.data[0].coupon_valid_from)
            )
        );
        setTransactionDate2(
            dayjs(
              moment(res.data[0].coupon_valid_till)
            )
        );
      });
  }, [couponId]);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      await axios.put(
        import.meta.env.VITE_BACKEND + "/api/admin/updateProListingCoupon",
        couponData
      );
      setLoader(false);
...
export default EditProPlanCoupon;
